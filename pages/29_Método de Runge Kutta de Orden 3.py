...
st.title("Método de Runge Kutta de Orden 3")
st.markdown("""Para $n=3$, se se puede hacer un desarrollo similar al método del segundo 
            orden. El resultado de dicho desarrollo es de 6 ecuaciones con ocho incógnitas.
            Por tano, se debe especificar con antelación los valores para las dos incógnitas
            con el fin de establecer los parámetros resultantes. Una versión común resulta 
            que es """)

st.markdown("La fórmula para calcular el siguiente queda:")
st.latex(r"y_{i+1} = y_i + \frac{1}{6}(k_1 + 4k_2 + k_3)h")
st.markdown("Donde: ")
st.latex(r"k_1 = f(x_i,y_i)")
st.latex(r"k_2 = f(x_i+\frac{1}{2}h, y_i +\frac{1}{2}k_1h)")
st.latex(r"k_3 = f(x_i+h, y_i - k_1h + 2k_2h)")

st.markdown("""A continuación, emplearemos el método de Runge-Kutta de orden 3 para la solución del
...
def runge_kutta_3(funcs, y0, t0, tf, dt):
    ...
    for t in t_values[:-1]:
        ...
        k3 = np.array([f(*(y_current - k1 * dt + 2 * k2 * dt), t + dt) for f in funcs])

        y_next = y_current + (1/6) * dt * (k1 + 4 * k2 + k3)
...
t_values_rk3, [v1, v2] = runge_kutta_3(
    [func_a, func_b], initial_conditions, t0, tf, dt
)

fig, axs = plt.subplots(1,2,figsize=(20,6))

axs[0].plot(t_values_rk3, v1, label=f'Aproximado ${a}$(t)', linestyle='--')
...
axs[1].plot(t_values_rk3, v2, label=f'Aproximado ${b}$(t)', linestyle='--', color='red')
